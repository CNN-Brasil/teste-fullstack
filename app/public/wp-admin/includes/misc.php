...
function save_mod_rewrite_rules()
{
     ...
     */
    if (
        ! file_exists($htaccess_file) && is_writable($home_path) && $wp_rewrite->using_mod_rewrite_permalinks()
...
function wp_doc_link_parse($content)
{
    ...
    for ($t = 0; $t < $count - 2; $t++) {
        ...
        if (T_STRING === $tokens[ $t ][0] && ( '(' === $tokens[ $t + 1 ] || '(' === $tokens[ $t + 2 ] )) {
            ...
            if (
                ( isset($tokens[ $t - 2 ][1]) && in_array($tokens[ $t - 2 ][1], array( 'function', 'class' ), true) )

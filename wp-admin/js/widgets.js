...
(function ($) {
	...
	window.wpWidgets = {
		...
		init : function () {
			...
			$( window ).on(
				'beforeunload.widgets',
				function ( event ) {
					var dirtyWidgetIds = [], unsavedWidgetsElements;
					$.each(
						self.dirtyWidgets,
						function ( widgetId, dirty ) {
							if ( dirty ) {
					...
					if ( 0 !== dirtyWidgetIds.length ) {
						unsavedWidgetsElements = $( '#widgets-right' ).find( '.widget' ).filter(
							function () {
								return -1 !== dirtyWidgetIds.indexOf( $( this ).prop( 'id' ).replace( /^widget-\d+_/, '' ) );
							}
						);
						unsavedWidgetsElements.each(
							function () {
						...
						unsavedWidgetsElements.first().each(
							function () {
			...
			sidebars.sortable(
				{
					...
					receive: function ( event, ui ) {
						...
						if ( $sender.attr( 'id' ).indexOf( 'orphaned_widgets' ) > -1 && ! $sender.children( '.widget' ).length ) {
							$sender.parents( '.orphan-sidebar' ).slideUp(
								400,
								function () {
									$( this ).remove(); }
							);
			...
			// Area Chooser.
			$( '#widgets-right .widgets-holder-wrap' ).each(
			...
			$( '#available-widgets .widget .widget-top' ).on(
				'click.widgets-chooser',
				function () {
				...
				function (r) {
					...
					if ( del ) {
						if ( ! $( 'input.widget_number', widget ).val() ) {
							$( '#available-widgets' ).find( 'input.widget-id' ).each(
								function () {
									if ( $( this ).val() === id ) {
		...
		removeInactiveWidgets : function () {
			...
			$.post(
				...
				data,
				function () {
					$( '#wp_inactive_widgets .widget' ).each(
						function () {
							var $widget = $( this );
		...
		close : function (widget) {
			widget.children( '.widget-inside' ).slideUp(
				'fast',
				function () {
					widget.attr( 'style', '' )

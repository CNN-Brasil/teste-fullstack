	...
	revisions.model.Diffs = Backbone.Collection.extend(
		{
			...
			getClosestUnloaded: function ( ids, centerId ) {
				var self = this;
				return _.chain( [0].concat( ids ) ).initial().zip( ids ).sortBy(
					function ( pair ) {
			...
			_loadAll: function ( allRevisionIds, centerId, num ) {
				...
				if ( _.size( diffs ) > 0 ) {
					this.load( diffs ).done(
						function () {
							self._loadAll( allRevisionIds, centerId, num ).done(
								function () {
									deferred.resolve();
								}
							);
						...
						function () {
							if ( 1 === num ) { // Already tried 1. This just isn't working. Give up.
								...
								self._loadAll( allRevisionIds, centerId, Math.ceil( num / 2 ) ).done(
									function () {
										deferred.resolve();
									}
								);
			...
			load: function ( comparisons ) {
				...
				return this.fetch( { data: { compare: comparisons }, remove: false } ).done(
					function () {

...
( function ( api, wp, $ ) {
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = wp.ajax.post(
			'customize-nav-menus-insert-auto-draft',
			{
	...
	api.Menus.AvailableMenuItemsPanelView = wp.Backbone.View.extend(
		/** @lends wp.customize.Menus.AvailableMenuItemsPanelView.prototype */        {
			...
			initialize: function () {
				 ...
				 */
				$( '#customize-controls, .customize-section-back' ).on(
					'click keydown',
					function ( e ) {
				...
				api.panel( 'nav_menus' ).container.on(
					'expanded',
					function () {
						if ( ! self.rendered ) {
							self.initList();
			...
			open: function ( menuControl ) {
				...
				_( this.currentMenuControl.getMenuItemControls() ).each(
					function ( control ) {
	...
	api.Menus.MenusPanel = api.Panel.extend(
		/** @lends wp.customize.Menus.MenusPanel.prototype */        {
			...
			ready: function () {
				var panel = this;
				panel.container.find( '.hide-column-tog' ).on(
					'click',
					function () {
			 ...
			 * @return {void}
			 */
			saveManageColumnsState: _.debounce(
				function () {
					var panel = this;
					...
					panel._updateHiddenColumnsRequest = wp.ajax.post(
						'hidden-columns',
						{
							hidden: panel.hidden(),
					...
					panel._updateHiddenColumnsRequest.always(
						function () {
		 ...
		 */
		hidden: function () {
			return $( '.hide-column-tog' ).not( ':checked' ).map(
				function () {
					var id = this.id;
	...
	api.Menus.MenuSection = api.Section.extend(
		/** @lends wp.customize.Menus.MenuSection.prototype */        {
			...
			ready: function () {
				...
				section.assignedLocations.bind(
					function ( to ) {
			...
			populateControls: function () {
				...
				if ( ! menuNameControl ) {
					menuNameControl = new api.controlConstructor.nav_menu_name(
						menuNameControlId,
						{
							type: 'nav_menu_name',
				...
				if ( ! menuControl ) {
					menuControl = new api.controlConstructor.nav_menu(
						section.id,
						{
							type: 'nav_menu',
							section: section.id,
				...
				if ( ! menuDeleteControl ) {
					menuDeleteControl = new api.Control(
						menuDeleteControlId,
						{
							section: section.id,
							priority: 1001,
					...
					menuDeleteControl.deferred.embedded.done(
						function () {
							menuDeleteControl.container.find( 'button' ).on(
								'click',
								function () {
			...
			refreshAssignedLocations: function () {
				...
				_.each(
					section.navMenuLocationSettings,
					function ( setting, themeLocation ) {
			...
			onChangeExpanded: function ( expanded, args ) {
				...
				if ( expanded ) {
					...
					_.each(
						api.section( section.id ).controls(),
						function ( control ) {
		...
		/** @lends wp.customize.Menus.NewMenuSection.prototype */        {
			...
			populateControls: function () {
				...
				if ( ! newMenuSubmitControl ) {
					newMenuSubmitControl = new api.Control(
						newMenuSubmitControlId,
						{
							section: section.id,
							priority: 1,
			...
			submit: function () {
				...
				contentContainer.find( '.assigned-menu-location input[type=checkbox]' ).each(
					function () {
						var checkbox = $( this ),
	...
	api.Menus.MenuLocationControl = api.Control.extend(
		/** @lends wp.customize.Menus.MenuLocationControl.prototype */        {
			...
			ready: function () {
				...
				control.container.find( '.create-menu' ).on(
					'click',
					function () {
				...
				control.container.find( '.edit-menu' ).on(
					'click',
					function () {
	...
	api.Menus.MenuItemControl = api.Control.extend(
		/** @lends wp.customize.Menus.MenuItemControl.prototype */        {
			...
			_setupControlToggle: function () {
				...
				this.container.find( '.menu-item-handle' ).on(
					'click',
					function ( e ) {
						e.preventDefault();
						e.stopPropagation();
			...
			_setupReorderUI: function () {
				...
				$reorderNav.find( '.menus-move-up, .menus-move-down, .menus-move-left, .menus-move-right' ).on(
					'click',
					function () {
						var moveBtn          = $( this );
				...
				control.setting.bind(
					function ( to, from ) {
						...
						if ( false === to ) {
							...
							_.each(
								menuControl.getMenuItemControls(),
							...
							_.each(
								followingSiblingItemControls,
								function ( followingSiblingItemControl ) {
							...
							_.each(
								childrenItemControls,
								function ( childrenItemControl, i ) {
			...
			_setupTitleUI: function () {
				...
				control.container.find( '.edit-menu-item-title' ).on(
					'blur',
					function () {
			...
			_changePosition: function ( offset ) {
				...
				_( control.getMenuControl().getMenuItemControls() ).each(
					function ( otherControl ) {
			...
			_changeDepth: function ( offset ) {
				...
				if ( -1 === offset ) {
					...
					_( siblingControls ).chain().slice( realPosition ).each(
						function ( siblingControl, i ) {
	...
	api.Menus.MenuLocationsControl = api.Control.extend(
		/** @lends wp.customize.Menus.MenuLocationsControl.prototype */        {
			...
			ready: function () {
				...
				control.container.find( '.assigned-menu-location' ).each(
					function () {
						var container           = $( this ),
			...
			setSelections: function ( selections ) {
				this.container.find( '.menu-location' ).each(
					function ( i, checkboxNode ) {
		...
		/** @lends wp.customize.Menus.MenuControl.prototype */        {
			...
			ready: function () {
				...
				api.section(
					control.section(),
					function ( section ) {
						section.deferred.initSortables.done(
							function ( menuList ) {
			...
			_setupSortable: function ( menuList ) {
					...
					function () {
						setTimeout(
							function () {
								...
								_.each(
									menuItemContainerIds,
									function ( menuItemContainerId ) {
										var menuItemId, menuItemControl, matches;
				 ...
				 */
				this.container.find( '.reorder-toggle' ).on(
					'click',
					function () {
			...
			_setupAddition: function () {
				...
				this.container.find( '.add-new-menu-item' ).on(
					'click',
					function ( event ) {
			...
			_setupTitle: function () {
				...
				control.setting.bind(
					function ( menu ) {
						...
						section.contentContainer.find( '.customize-control-checkbox input' ).each(
							function () {
			...
			toggleReordering: function ( showOrHide ) {
				...
				if ( showOrHide ) {
					_( this.getMenuItemControls() ).each(
			...
			reflowMenuItems: function () {
				...
				reflowRecursively = function ( context ) {
					...
					if ( currentMenuItemControls.length ) {
						_( currentMenuItemControls ).each(
							function ( menuItemControl ) {
			 ...
			 */
			debouncedReflowMenuItems: _.debounce(
				function () {
	...
	api.bind(
		...
		function () {
			...
			*/
			api.state( 'changesetStatus' ).bind(
				function ( status ) {
					if ( 'publish' === status ) {
	...
	api.Menus.applySavedData = function ( data ) {
		...
		_( data.nav_menu_updates ).each(
			function ( update ) {
				...
				if ( 'inserted' === update.status ) {
					...
					newSection = new api.Menus.MenuSection(
						newCustomizeId,
						{
							panel: 'nav_menus',
		...
		_( data.nav_menu_item_updates ).each(
			function ( update ) {
		...
		_( data.nav_menu_item_updates ).each(
			function ( update ) {
				...
				if ( 'inserted' === update.status ) {
					...
					// Add the menu control.
					newControl = new api.controlConstructor.nav_menu_item(
						newCustomizeId,
						{
							type: 'nav_menu_item',
		...
		_.each(
			...
			function ( widgetSettingValue, widgetSettingId ) {

...
(function ( wp, $ ) {
	...
	api.Widgets.AvailableWidgetsPanelView = wp.Backbone.View.extend(
		...
		updateSearchMatchesCount: function () {
			...
			open: function ( sidebarControl ) {
				...
				_( this.currentSidebarControl.getWidgetFormControls() ).each(
					function ( control ) {
		...
		/** @lends wp.customize.Widgets.WidgetControl.prototype */        {
			...
			_setupReorderUI: function () {
				...
				updateAvailableSidebars = function () {
					...
					selfSidebarItem = $sidebarItems.filter(
						function () {
				...
				$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each(
					function () {
				 ...
				 */
				this.container.find( '.widget-area-select' ).on(
					'click keypress',
					'li',
					function ( event ) {
						if ( event.type === 'keypress' && ( event.which !== 13 && event.which !== 32 ) ) {
				 ...
				 */
				this.container.find( '.move-widget-btn' ).click(
					function () {
			...
			_setupUpdateUI: function () {
				...
				updateWidgetDebounced = _.debounce(
					function () {
						self.updateWidget();
					},
					250
				...
				this.setting.previewer.channel.bind(
					'synced',
					function () {
			...
			_getInputsSignature: function ( inputs ) {
				var inputsSignatures = _( inputs ).map(
					function ( input ) {
			...
			_getInputState: function ( input ) {
				...
				if ( input.is( ':radio, :checkbox' ) ) {
					...
					return input.find( 'option:selected' ).map(
						function () {
							return $( this ).val();
						}
					).get();
			...
			onChangeExpanded: function ( expanded, args ) {
				...
				if ( args.unchanged ) {
					if ( expanded ) {
						api.Control.prototype.expand.call(
							self,
							{
	...
	api.Widgets.SidebarControl = api.Control.extend(
		/** @lends wp.customize.Widgets.SidebarControl.prototype */        {
			...
			_setupModel: function () {
				...
				this.setting.bind(
					function ( newWidgetIds, oldWidgetIds ) {
						...
						newWidgetIds = _( newWidgetIds ).filter(
						...
						widgetFormControls = _( newWidgetIds ).map(
							function ( widgetId ) {
			...
			_setupAddition: function () {
				...
				this.container.find( '.add-new-widget' ).on(
					'click',
					function () {
			...
			toggleReordering: function ( showOrHide ) {
				...
				if ( showOrHide ) {
					_( this.getWidgetFormControls() ).each(
			...
			addWidget: function ( widgetId ) {
				...
				widgetFormControl  = new controlConstructor(
					settingId,
					{
						settings: {
							'default': settingId
	...
	$( document ).on(
		'widget-added',
		function ( event, widgetContainer ) {

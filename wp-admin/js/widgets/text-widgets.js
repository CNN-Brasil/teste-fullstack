		...
		/** @lends wp.textWidgets.TextWidgetControl.prototype */        {
			...
			initialize: function initialize( options ) {
				...
				if ( control.customHtmlWidgetPointer.length ) {
					control.customHtmlWidgetPointer.find( '.close' ).on(
						'click',
						function ( event ) {
				...
				if ( control.pasteHtmlPointer.length ) {
					control.pasteHtmlPointer.find( '.close' ).on(
						'click',
						function ( event ) {
							event.preventDefault();
			...
			initializeEditor: function initializeEditor() {
				...
				control.syncContainer.closest( '.widget' ).find( '[name=savewidget]:first' ).on(
					'click',
				...
				function buildEditor() {
					...
					$( document ).one(
						'wp-before-tinymce-init.text-widget-init',
						function ( event, init ) {

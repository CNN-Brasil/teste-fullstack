...
(function ($) {
	...
	api = window.wpNavMenu = {
			...
			$.fn.extend(
				{
					...
					hideAdvancedMenuItemFields : function () {
						return this.each(
							function () {
								var that = $( this );
								$( '.hide-column-tog' ).not( ':checked' ).each(
									function () {
		...
		refreshAdvancedAccessibility : function () {
			...
			$( '.menu-item-edit-active a.item-edit' ).each(
				function () {
					api.refreshAdvancedAccessibilityOfItem( this );
				}
			);
		...
		initToggles : function () {
			...
			$( '.hide-postbox-tog' ).on(
				'click',
				function () {
					var hidden = $( '.accordion-container li.accordion-section' ).filter( ':hidden' ).map(
						function () {
							return this.id; }
					).get().join( ',' );
					$.post(
		...
		initManageLocations : function () {
			$( '#menu-locations-wrap form' ).on(
				'submit',
				function () {
					window.onbeforeunload = null;
				}
			);
			$( '.menu-location-menus select' ).on(
				'change',
				function () {
		...
		attachMenuEditListeners : function () {
			...
			$( '#add-custom-links input[type="text"]' ).on(
				'keypress',
				function (e) {
		...
		attachThemeLocationsListeners : function () {
			...
			loc.find( 'input[type="submit"]' ).on(
				'click',
				function () {
					loc.find( 'select' ).each(
						function () {
		...
		attachUnsavedChangesListener : function () {
			$( '#menu-management input, #menu-management select, #menu-management, #menu-management textarea, .menu-location-menus select' ).on(
				'change',
				function () {
					api.registerChange();
				}
		...
		eventOnClickMenuSave : function () {
			...
			$( '#nav-menu-theme-locations select' ).each(
				function () {
			...
			api.menuList.find( '.menu-item-data-position' ).val(
				function (index) {
					return index + 1; }
			);
			window.onbeforeunload = null;
	...
	$(
		function () {
			...
			$( '.menu-edit a, .menu-edit button, .menu-edit input, .menu-edit textarea, .menu-edit select' ).on(
				'focus',
				function () {

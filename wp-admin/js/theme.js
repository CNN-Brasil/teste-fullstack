			...
			*/
			query: function ( request ) {
				...
				if ( ! query && ! isPaginated ) {
					query = this.apiCall( request ).done(
						function ( data ) {
					...
					if ( isPaginated ) {
						return this.apiCall( request, isPaginated ).done(
							function ( data ) {
	...
	themes.view.Details = wp.Backbone.View.extend(
		{
			...
			deleteTheme: function ( event ) {
				...
				$( document ).one(
					'wp-theme-delete-success',
					function ( event, response ) {
						_this.$el.find( '.close' ).trigger( 'click' );
						$( '[data-slug="' + response.slug + '"]' ).css( { backgroundColor:'#faafaa' } ).fadeOut(
							350,
							function () {
								$( this ).remove();
			...
			render: function () {
				...
				this.listenTo(
					...
					function () {
						...
						$( '.theme-browser .error .try-again' ).on(
							'click',
							function ( e ) {
								e.preventDefault();

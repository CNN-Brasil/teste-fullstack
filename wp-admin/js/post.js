...
(function ($) {
	...
	$(
		...
		'heartbeat-send.wp-refresh-nonces',
		function ( e, data ) {
			var post_id,
		...
		'heartbeat-tick.wp-refresh-nonces',
		function ( e, data ) {
...
jQuery(
	function ($) {
		...
		$( '#post-lock-dialog .notification-dialog' ).on(
			'keydown',
			function (e) {
		...
		$submitButtons = $submitpost.find( ':submit, a.submitdelete, #post-preview' ).on(
			'click.edit-post',
			function ( event ) {
				var $button = $( this );
				...
				$( 'form#post' ).off( 'submit.edit-post' ).on(
					'submit.edit-post',
					function ( event ) {
		...
		$document.on(
			'autosave-disable-buttons.edit-post',
			function () {
				$submitButtons.addClass( 'disabled' );
			}
		).on(
			'autosave-enable-buttons.edit-post',
			function () {
			...
			'before-autosave.edit-post',
			function () {
				$( '.autosave-message' ).text( __( 'Saving Draft…' ) );
			}
		).on(
			'after-autosave.edit-post',
			function ( event, data ) {
		...
		if ( $( '#tagsdiv-post_tag' ).length ) {
			window.tagBox && window.tagBox.init();
				...
				$( '.meta-box-sortables' ).children( 'div.postbox' ).each(
					function () {
		...
		if ( $( '#submitdiv' ).length ) {
			...
			$( '#visibility .edit-visibility' ).on(
				'click',
				function ( e ) {
					e.preventDefault();
					if ( $postVisibilitySelect.is( ':hidden' ) ) {
						updateVisibility();
						$postVisibilitySelect.slideDown(
							'fast',
							function () {
				...
				$postVisibilitySelect.find( '.cancel-post-visibility' ).on(
					'click',
					function ( event ) {
				...
				$postVisibilitySelect.find( '.save-post-visibility' ).on(
					'click',
					function ( event ) {
				...
				$postVisibilitySelect.find( 'input:radio' ).on(
					'change',
					function () {
						updateVisibility();
					}
				...
				$timestampdiv.siblings( 'a.edit-timestamp' ).on(
					'click',
					function ( event ) {
				...
				$timestampdiv.find( '.cancel-timestamp' ).on(
					'click',
					function ( event ) {
				...
				$timestampdiv.find( '.save-timestamp' ).on(
					'click',
					function ( event ) {
						// Crazyhorse branch - multiple OK cancels.
						if ( updateText() ) {
				...
				$postStatusSelect.siblings( 'a.edit-post-status' ).on(
					'click',
					function ( event ) {
						if ( $postStatusSelect.is( ':hidden' ) ) {
									$postStatusSelect.slideDown(
										'fast',
										function () {
				...
				$postStatusSelect.find( '.save-post-status' ).on(
					'click',
					function ( event ) {
				...
				$postStatusSelect.find( '.cancel-post-status' ).on(
					'click',
					function ( event ) {
						$postStatusSelect.slideUp( 'fast' ).siblings( 'a.edit-post-status' ).show().trigger( 'focus' );
		...
		if ( typeof tinymce !== 'undefined' ) {
			...
			$( '#post-formats-select input.post-format' ).on(
				'change.set-editor-class',
				function () {

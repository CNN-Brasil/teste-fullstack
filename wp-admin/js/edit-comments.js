...
(function ($) {
			...
			if ( action ) {
				...
				a.on(
					...
					function ( e ) {
						...
						$( '#undo-' + id ).css( {backgroundColor:'#ceb'} ).fadeOut(
							350,
							function () {
								$( this ).remove();
								$( '#comment-' + id ).css( 'backgroundColor', '' ).fadeIn(
									300,
									function () {
										$( this ).show(); }
								);
	...
	window.commentReply = {
		...
		init : function () {
			...
			// Add events.
			$( '#the-comment-list .column-comment > p' ).on(
				'dblclick',
				function () {
		...
		send : function () {
			...
			$( '#replyrow input' ).not( ':button' ).each(
				function () {
					var t                    = $( this );

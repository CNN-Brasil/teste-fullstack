...
(function ($) {
	...
	window.postboxes = {
		...
		handleOrderBetweenSortables: function ( position, button, postbox ) {
			...
			$( '.meta-box-sortables:visible' ).each(
				function () {
		...
		save_state : function (page) {
			...
			closed = $( '.postbox' ).filter( '.closed' ).map(
				function () {
					return this.id; }
			).get().join( ',' );
			hidden = $( '.postbox' ).filter( ':hidden' ).map(
				function () {
					return this.id; }
			).get().join( ',' );

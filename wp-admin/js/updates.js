...
(function ( $, wp, settings ) {
	...
	wp.updates.refreshCount = function () {
		...
		$dashboardNavMenuUpdateCount.each(
		...
		$pluginsNavMenuUpdateCount.each(
			function ( index, element ) {
		...
		$appearanceNavMenuUpdateCount.each(
			function ( index, element ) {
	...
	wp.updates.deletePluginSuccess = function ( response ) {
		...
		$( '[data-plugin="' + response.plugin + '"]' ).css( { backgroundColor: '#faafaa' } ).fadeOut(
			350,
			function () {
		...
		setTimeout(
			function () {
				...
				if ( response.customizeUrl ) {
					...
					$message.siblings( '.preview' ).replaceWith(
						function () {
							return $( '<a>' )
	...
	wp.updates.deleteThemeSuccess = function ( response ) {
		...
		if ( 'themes-network' === pagenow ) {
			...
			$themeRows.css( { backgroundColor: '#faafaa' } ).fadeOut(
				350,
				function () {
					var $views = $( '.subsubsub' ),
					$themeRow  = $( this ),
	...
	$(
		function () {
			...
			$document.on(
				'click',
				'#plugin-information-footer .activate-now',
				function ( event ) {
					event.preventDefault();
			...
			$document.on(
				'click',
				'.themes-php.network-admin .update-link',
				function ( event ) {
					var $message = $( event.target ),
			...
			$document.on(
				'click',
				'.themes-php.network-admin a.delete',
				function ( event ) {
				...
				_.debounce(
					function ( event, eventtype ) {
						...
						if ( 'undefined' !== typeof wp.updates.searchRequest ) {
							...
							wp.updates.searchRequest = wp.ajax.post( 'search-install-plugins', data ).done(
								function ( response ) {
				...
				_.debounce(
					function ( event ) {
						...
						if ( 'undefined' !== typeof wp.updates.searchRequest ) {
							...
							wp.updates.searchRequest = wp.ajax.post( 'search-plugins', data ).done(
								function ( response ) {

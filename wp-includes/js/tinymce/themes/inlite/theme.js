(function () {
	var inlite = (function (domGlobals) {
			...
			add: function (ctrl) {
				...
				if (parent) {
					...
					if ( ! animationFrameRequested) {
						...
						global$3.requestAnimationFrame(
							function () {
								var id, ctrl;
					...
					if (settings.tooltip && Widget.tooltips !== false) {
						self.on(
							...
							function (e) {
								...
								if (e.control === self) {
										var rel = tooltip.text( settings.tooltip ).show().testMoveRel(
											self.getEl(),
											[
											'bc-tc',
											'bc-tl',
		...
		var Container     = Control$1.extend(
			{
				init: function (settings) {
					...
					self.bodyClasses        = new ClassList(
						function () {
							if (self.state.get( 'rendered' )) {
			...
			renderScroll: function () {
				...
				function addScroll() {
					function addScrollAxis(axisName, posName, sizeName, deltaPosName, ax) {
						...
						self.draghelper = new DragHelper(
							axisId + 't',
							{
								start: function () {
							...
							function () {
								...
								if (self._fullscreen) {
									if ( ! slowRendering) {
										...
										if ( ! self._timer) {
											self._timer = global$3.setTimeout(
												function () {
													var rect = funcs.getWindowSize();
													self.moveTo( 0, 0 ).resizeTo( rect.w, rect.h );
													self._timer = 0;
		...
		var addToEditor      = function (editor, panel) {
			...
			editor.addButton(
				'quickimage',
				{
					...
					onclick: function () {
						Picker.pickFile().then(
							function (files) {
								var blob = files[0];
								Conversions.blobToBase64( blob ).then(
									function (base64) {
		...
		var createQuickLinkForm   = function (editor, hide) {
			...
			return createForm(
				'quicklink',
				{
					...
					onsubmit: function (e) {
						convertLinkToAbsolute( editor, e.data.linkurl ).then(
							function (url) {
								editor.undoManager.transact(
									function () {
		...
		var create$3 = function () {
			...
			var focus               = function () {
				if (panel) {
					panel.find( 'toolbar:visible' ).eq( 0 ).each(
						function (item) {
							item.focus( true );
						}
					);
		...
		var AbsoluteLayout = Layout$1.extend(
			{
				...
				recalc: function (container) {
					container.items().filter( ':visible' ).each(
						function (ctrl) {
					...
					self.on(
						...
						function (e) {
							...
							if (self.statusLevel() && e.target.className.indexOf( self.classPrefix + 'status' ) !== -1) {
								...
								var rel           = tooltip.text( statusMessage ).show().testMoveRel(
									e.target,
									[
									'bc-tc',
									'bc-tl',
		...
		var PanelButton = Button.extend(
			{
				showPanel: function () {
					...
					if ( ! self.panel) {
						...
						self.panel             = new FloatPanel( panelSettings ).on(
							'hide',
							function () {
		...
		var ColorPicker = Widget.extend(
			{
				...
				postRender: function () {
					...
					self._super();
					self._svdraghelper = new DragHelper(
						self._id + '-sv',
						{
					...
					self._hdraghelper  = new DragHelper(
						self._id + '-h',
						{
							start: updateHue,
							drag: updateHue
		...
		var detectBrowser = function (browsers, userAgent) {
			return detect$1( browsers, userAgent ).map(
				function (browser) {
		...
		var detectOs      = function (oses, userAgent) {
			return detect$1( oses, userAgent ).map(
				function (os) {
		...
		var select                       = function (selector, root) {
			return map(
				descendants( Element.fromDom( root ), selector ),
				function (element) {
					return element.dom();
				}
			);
		...
		var setupUiContainer     = function (editor) {
			if (editor.settings.ui_container) {
				global$1.container = descendant( Element.fromDom( domGlobals.document.body ), editor.settings.ui_container ).fold(
					constant( null ),
					function (elm) {
						return elm.dom();
					}
					...
					self.on(
						...
						function (e) {
							...
							if (overCtrl && parent && overCtrl instanceof MenuButton && overCtrl.parent() === parent) {
								parent.items().filter( 'MenuButton' ).each(
									function (ctrl) {
		...
		var ResizeHandle = Widget.extend(
			{
				...
				postRender: function () {
					var self = this;
					self._super();
					self.resizeDragHelper = new DragHelper(
						this._id,
						{
							start: function () {
		...
		var Slider = Widget.extend(
			{
				...
				postRender: function () {
					...
					function handleDrag(minValue, maxValue, handleEl) {
						...
						self._dragHelper = new DragHelper(
							self._id,
							{

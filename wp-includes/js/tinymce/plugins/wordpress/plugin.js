		...
		function ( editor ) {
					...
					if ( editor.getParam( 'wp_paste_filters', true ) ) {
						...
						editor.on(
							...
							function ( event ) {
								...
								if ( tinymce.isIE ) {
									editor.$( 'a', event.node ).find( 'font, u' ).each(
										function ( i, node ) {
			...
			editor.on(
				...
				function () {
					if ( editor.theme.panel ) {
						each(
							[ 'button', 'colorbutton', 'splitbutton' ],
							function ( buttonType ) {
			...
			function prepareTooltips() {
				...
				if ( editor.settings.wp_shortcut_labels ) {
					each(
						editor.settings.wp_shortcut_labels,
						function ( value, tooltip ) {
			...
			function addShortcutsToListbox() {
				...
				each(
					editor.theme.panel.find( 'listbox' ),
					function ( listbox ) {
						if ( listbox && listbox.settings.text === 'Paragraph' ) {
							each(
								listbox.settings.values,
								function ( item ) {

	...
	$.table_hotkeys             = function (table, keys, opts) {
		...
		set_current_row       = function (tr) {
			if ($.table_hotkeys.current_row) {
				$.table_hotkeys.current_row.removeClass( selected_class );
			}
		...
		check                 = function () {
			$( opts.checkbox_expr, $.table_hotkeys.current_row ).each(
				function () {
		...
		make_key_callback     = function (expr) {
			return function () {
				if ( null == $.table_hotkeys.current_row ) {
					return false;
				}
				...
				if (clickable.is( '.' + destructive_class )) {
					next_row() || prev_row();
				}

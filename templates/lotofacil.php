...
>
  ...
  if (isset($data['premiacoes']) && is_array($data['premiacoes'])) {
    foreach ($data['premiacoes'] as $premiacao) {
      // Verifica se a faixa é '15 acertos' e atribui o valor do prêmio
      if ($premiacao['descricao'] === '15 acertos') {
        $faixa = $premiacao['descricao'];
        $valorPremio = $premiacao['valorPremio'];
        break; // Encerra o loop após encontrar o valor de '15 acertos'
      }
    }
  }

  // Exibe a mensagem com base no valor do prêmio
  if ($faixa === '15 acertos' && isset($data['premiacoes']) && $data['premiacoes'][0]['ganhadores'] == 0): ?>
    <p class="acumulou">

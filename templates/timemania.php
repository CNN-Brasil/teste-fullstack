...
>
  ...
  <div class="time-coracao">
    <p>Time do Coração: <strong><?php echo esc_html($data['timeCoracao']); ?></strong></p>
  </div>
  ...
  if (isset($data['premiacoes']) && is_array($data['premiacoes'])) {
    foreach ($data['premiacoes'] as $premiacao) {
      // Verifica se a faixa é '7 acertos' e atribui o valor do prêmio
      if ($premiacao['descricao'] === '7 acertos') {
        $faixa = $premiacao['descricao'];
        $valorPremio = $premiacao['valorPremio'];
        break; // Encerra o loop após encontrar o valor de '7 acertos'
      }
    }
  }

  // Exibe a mensagem com base no valor do prêmio
  if ($faixa === '7 acertos' && isset($data['premiacoes']) && $data['premiacoes'][0]['ganhadores'] == 0): ?>
    <p class="acumulou">

...
>
  ...
  <span class="trevos">Trevos</span>
  <div class="dezenas-sorteadas">    
    <?php if (isset($data['trevos']) && is_array($data['trevos'])): ?>
    <?php foreach ($data['trevos'] as $trevo): ?>
    <div class="dezena"><?php echo esc_html($trevo); ?></div>
    <?php endforeach; ?>
    <?php else: ?>
    <div class="dezena indisponivel">Trevos não disponíveis</div>
    <?php endif; ?>
  </div>
  ...
  if (isset($data['premiacoes']) && is_array($data['premiacoes'])) {
    foreach ($data['premiacoes'] as $premiacao) {
      // Verifica se a faixa é '6 acertos + 2 trevos' e atribui o valor do prêmio
      if ($premiacao['descricao'] === '6 acertos + 2 trevos') {
        $faixa = $premiacao['descricao'];
        $valorPremio = $premiacao['valorPremio'];
        break; // Encerra o loop após encontrar o valor de '6 acertos + 2 trevos'
      }
    }
  }

  // Exibe a mensagem com base no valor do prêmio
  if ($faixa === '6 acertos + 2 trevos' && isset($data['premiacoes']) && $data['premiacoes'][0]['ganhadores'] == 0): ?>
    <p class="acumulou">
